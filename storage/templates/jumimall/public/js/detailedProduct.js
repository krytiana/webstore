...
document.addEventListener("DOMContentLoaded", () => {
  ...
  productBtn?.addEventListener("click", async () => {
    ...
    // not logged in user case
    if (res.status === 401) {
      const data = await res.json();
      window.location.href = `/register?message=${encodeURIComponent(data.message + ", Please log in to add to cart.")}`;
      return;
    }
  ...
  document.querySelector(".wishlist")?.addEventListener("click", async (e) => {
    ...
    if (res.status === 401) {
      const data = await res.json();
      window.location.href = `/register?message=${encodeURIComponent(data.message + ", Please log in to add to wishlist.")}`;
      return;
    }

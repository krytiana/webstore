...
JumiMall is designed as a base product for Code Carthub. Each client should receive their own configuration, production environment, credentials, payment accounts, database, and deployment.

License: JumiMall is proprietary software owned by Code Carthub. Client use is governed by the applicable commercial license. See LICENSE.md.
...
JumiMall does not trust payment totals supplied by the browser.
...
JumiMall is designed to be customized for each client.
...
JumiMall is intended to be deployed separately for each client.
...
Because JumiMall is a reusable product, keep the original Code Carthub source separate from individual client deployments.
...
JumiMall is proprietary software owned by Code Carthub.

Clients may receive source code and may modify their licensed deployment according to the applicable commercial license, but receiving source code does not transfer ownership of the underlying reusable JumiMall software.
...
Product: JumiMall Business Store v2

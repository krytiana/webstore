...
TemuMall is designed as a base product for Code Carthub. Each client should receive their own configuration, production environment, credentials, payment accounts, database, and deployment.

License: TemuMall is proprietary software owned by Code Carthub. Client use is governed by the applicable commercial license. See LICENSE.md.
...
TemuMall does not trust payment totals supplied by the browser.
...
TemuMall is designed to be customized for each client.
...
TemuMall is intended to be deployed separately for each client.
...
Because TemuMall is a reusable product, keep the original Code Carthub source separate from individual client deployments.
...
TemuMall is proprietary software owned by Code Carthub.

Clients may receive source code and may modify their licensed deployment according to the applicable commercial license, but receiving source code does not transfer ownership of the underlying reusable TemuMall software.
...
Product: TemuMall Business Store v2

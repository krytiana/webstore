...
StickyCat is designed as a base product for Code Carthub. Each client should receive their own configuration, production environment, credentials, payment accounts, database, and deployment.

License: StickyCat is proprietary software owned by Code Carthub. Client use is governed by the applicable commercial license. See LICENSE.md.
...
StickyCat does not trust payment totals supplied by the browser.
...
StickyCat is designed to be customized for each client.
...
StickyCat is intended to be deployed separately for each client.
...
Because StickyCat is a reusable product, keep the original Code Carthub source separate from individual client deployments.
...
StickyCat is proprietary software owned by Code Carthub.

Clients may receive source code and may modify their licensed deployment according to the applicable commercial license, but receiving source code does not transfer ownership of the underlying reusable StickyCat software.
...
Product: StickyCat Business Store v2
